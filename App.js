...
function AppNavigator() {
  ...
  const { isAuthenticated, loading, isHydrated } = useSelector(
    (state) => state.auth
  );
  // const dispatch = useDispatch();
console.log("roles is ", role,  isAuthenticated);
  React.useEffect(() => {
    if (isAuthenticated) {
  ...
  return (
    <NavigationContainer>
      <Stack.Navigator screenOptions={{ headerShown: false }}>
        {(!isAuthenticated || role === roles.none) ? (
          <>
